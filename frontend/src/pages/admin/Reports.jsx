...
const ReportsAdmin = () => {
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      link.setAttribute('download', `hr_admin_${reportType}_report.csv`);
    ...
    <div className="max-w-3xl mx-auto space-y-4">
      <div className="glass-card p-4 p-md-5">
        <h4 className="fw-bold text-primary mb-1">
          <i className="bi bi-file-earmark-spreadsheet me-2"></i>Export Organization Reports
        </h4>
        <p className="text-muted small mb-4">Generate and download official CSV reports for audits, leaves, and expenses.</p>
        ...
        <div className="row g-3 mb-4">
          <div className="col-12">
            ...
            <select className="form-select" value={reportType} onChange={(e) => setReportType(e.target.value)}>
              <option value="leaves">Company Employee Leaves Master Report</option>
              <option value="expenses">Company Expense Reimbursements Master Report</option>
            </select>
          </div>
        ...
        <button className="btn btn-primary fw-bold shadow w-100 py-2.5" onClick={handleExportCSV} disabled={downloading}>
          ...
          {downloading ? 'Generating CSV...' : 'Download Master CSV Report'}
        </button>
      </div>
    </div>
  );
