...
const ExpensesAdmin = () => {
  ...
  const handleUpdateStatus = async (id, status) => {
    try {
      ...
      setMsg(`Expense claim marked as ${status}.`);
      loadExpenses();
    } catch (err) {
  ...
  if (loading) return <Loading message="Loading expense claims master..." />;

  return (
    <div className="space-y-4">
      <div>
        <h4 className="fw-bold text-primary mb-1">Company Expense Claims Master</h4>
        <p className="text-muted small mb-3">Manage organization expense reimbursements and payout statuses.</p>
      </div>
      ...
      <div className="glass-card p-4">
        {expenses.length === 0 ? (
          <p className="text-muted text-center py-4">No expense claims logged.</p>
        ) : (
